  ...
  return {
        ...
        for (var key in obj) {
          if (obj.hasOwnProperty(key)) {
            if (typeof obj[key] === "object")
              out[key] = ULUtil.deepExtend(out[key], obj[key]);
      ...
      if (el.classList) {
        ...
        for (var x = 0; x < classNames.length; x++) {
          el.className = el.className.replace(
            new RegExp("\\b" + ULUtil.trim(classNames[x]) + "\\b", "g"),
            ""
          );
        }
      }
    ...
    children: function (el, selector, log) {
      ...
      for (var i; i < l; ++i) {
        if (
          ...
          ULUtil.matches(el.childNodes[i], selector, log)
        ) {
    ...
    data: function (element) {
      ...
      return {
        set: function (name, data) {
          ...
          if (
            window.ULUtilElementDataStore[element.customDataTag] === undefined
          ) {
        ...
        get: function (name) {
          ...
          return this.has(name)
            ? window.ULUtilElementDataStore[element.customDataTag][name]
            : null;
        },
        ...
        has: function (name) {
          ...
          return window.ULUtilElementDataStore[element.customDataTag] &&
            window.ULUtilElementDataStore[element.customDataTag][name]
            ? true
            : false;
        },
    ...
    outerWidth: function (el, margin) {
      ...
      if (margin === true) {
        ...
        width +=
          parseFloat(ULUtil.css(el, "margin-left")) +
          parseFloat(ULUtil.css(el, "margin-right"));
    ...
    slide: function (el, dir, speed, callback, recalcMaxHeight) {
      if (
        !el ||
        (dir == "up" && ULUtil.visible(el) === false) ||
        (dir == "down" && ULUtil.visible(el) === true)
      ) {
        return;
      }
      ...
      if (
        ULUtil.css(el, "padding-top") &&
        ULUtil.data(el).has("slide-padding-top") !== true
      ) {
        ULUtil.data(el).set("slide-padding-top", ULUtil.css(el, "padding-top"));
      }

      if (
        ULUtil.css(el, "padding-bottom") &&
        ULUtil.data(el).has("slide-padding-bottom") !== true
      ) {
        ULUtil.data(el).set(
      ...
      if (ULUtil.data(el).has("slide-padding-bottom")) {
        calcPaddingBottom = parseInt(
          ULUtil.data(el).get("slide-padding-bottom")
        );
      }
    ...
    on: function (element, selector, event, handler) {
      ...
      ULUtil.addEvent(
        element,
        event,
        window.ULUtilDelegatedEventHandlers[eventId]
      );
    ...
    scrollInit: function (element, options) {
      ...
      function init() {
        ...
        if (
          ...
          ULUtil.isInResponsiveRange("tablet-and-mobile")
        ) {
        ...
        if (ps) {
          ...
          ps = new PerfectScrollbar(element, {
            ...
            suppressScrollX:
              ULUtil.attr(element, "data-scroll-x") != "true" ? true : false,
          });
