...
@section('custom_content')
    ...
    <div class="subheader px-lg">
        <div class="subheader-container">
            ...
            <div class="subheader-toolbar"><a class="btn btn-opacity-primary btn-sm mr-2" href="{{route('admin.events.show', $event->id)}}">Back</a>
            </div>
        </div>
    </div>
    ...
    <div class="container my-lg">
        <div class="row mb-md">
            <div class="col-12 text-center pt-l">
                <h1 class="mb-xl"> Edit Events</h1>
                @include('layouts.message')
            </div>
        </div>
        <form id="event-update-form" action="{{ route('events.update', $event->id) }}" method="post" data-id = {{$event->id}}>
            @method('PUT')
            @csrf
            <div class="doc-example">
                <div class="form-group ">
                    <input class="form-control" id="name" name="name" required value="{{ $event->name }}" autofocus
                ...
                <div class="d-flex justify-content-between">
                    <div class="form-group event">
                        ...
                        <input class="form-control" id="email" name="email" value="{{ $event->email }}" />
                    </div>
                    <div class="form-group event">
                        <label for="phone">Organizer's Phone <small class="text-danger">(optional)</small></label>
                        <input class="form-control" id="phone" name="phone" value="{{ $event->phone }}" />
                    </div>
                </div>

                <div class="form-group">
                    <label for="venue">Venue <small class="text-danger">(optional)</small></label>
                    <input class="form-control" id="venue" name="venue" value="{{ $event->venue }}" />
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="caption" name="caption">{{ $event->caption }}</textarea>
                </div>
